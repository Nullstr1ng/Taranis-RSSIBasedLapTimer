...
--
-- modified 2016-09-04 - PG techwit.org - to add lap time call outs and logical switch triggering to allow RSSI based laptimes, new sounds, and tidied up the included files
--
-- User Configuration
--
...
local SOUND_TIME = 'LAPTIME/time.wav'
...
local SWITCH_NAMES = { 'sa', 'sb', 'sc', 'sd', 'se', 'sf', 'sg', 'sh', "ls1", "ls2", "ls3", "ls4", "ls5", "ls6", "ls7", "ls8", "ls9" }
...
local function configuration_func(keyEvent)
	...
	lcd.drawText(38, 42, 'Speak Lap Time:')
...
local function race_setup_draw()
	...
	lcd.drawText (2,10," Laptimer V2")
	lcd.drawText (2,20," based on work by")
	lcd.drawText (2,30," Jeremy Cowgar")
	
	lcd.drawText (100,10," Menu - config")
	lcd.drawText (100,20," +    - more laps")
	lcd.drawText (100,30," -    - less laps")
	lcd.drawText (100,40," Enter- Go!")
	lcd.drawText (100,50," Hold exit - quit")
end
...
local function lapsSpeakProgress()
	...
	if #laps > 1 then
		...
		--	playNumber((thisLapTime-lastLapTime)/10, 24, PREC1)
